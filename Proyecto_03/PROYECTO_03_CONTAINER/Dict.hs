...
dict_empty :: Dict
dict_empty = Dict 0 (empty)



-- Pregunta si la clave esta en el diccionario
dict_exist :: Word -> Dict -> Bool
dict_exist palabra (Dict n l) = case (search (key_fromString palabra) l) of
                                     Just _ -> True
                                     Nothing -> False
...
dict_add palabra definicion (Dict n l) | dict_exist palabra (Dict n l) = (Dict n l)
                                       | otherwise = (Dict (n+1) (add (key_fromString palabra) (data_fromString definicion) l))
...
dict_search palabra (Dict n l) = convertidor (search (key_fromString palabra) l)
...
dict_del palabra (Dict n l) = case (search (key_fromString palabra) l) of
                                   (Just _) -> (Dict (n-1) (del (key_fromString palabra) l))
                                   Nothing -> (Dict n l)
...
dict_toDefs (Dict n l) = map nuevas (toListPair l)
