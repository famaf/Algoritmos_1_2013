...
dict_exist palabra (Dict n l) = case (la_search (key_fromString palabra) l) of
                                     Just _ -> True
                                     Nothing -> False
...
dict_add palabra definicion (Dict n l) | dict_exist palabra (Dict n l) = (Dict n l)
                                       | otherwise = (Dict (n+1) (la_add (key_fromString palabra) (data_fromString definicion) l))
...
dict_search palabra (Dict n l) = convertidor (la_search (key_fromString palabra) l)
...
dict_del palabra (Dict n l) = case (la_search (key_fromString palabra) l) of
                                   (Just _) -> (Dict (n-1) (la_del (key_fromString palabra) l))
                                   Nothing -> (Dict n l)
...
dict_toDefs (Dict n l) = map nuevas (la_toListPair l)
